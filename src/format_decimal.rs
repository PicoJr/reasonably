...
pub(crate) fn format_decimal_loc(decimal: Decimal) -> String {
    ...
    let stages: Vec<(Decimal, String)> = vec![
        (Decimal::new(1e3), "loc".to_string()),
        (Decimal::new(1e6), "Kiloloc".to_string()),
        (linux_kernel_loc, "Megaloc".to_string()),
        (linux_kernel_loc * Decimal::new(1e3), "Linux Kernels (LK)".to_string()),
        (linux_kernel_loc * Decimal::new(1e6), "KiloLK".to_string()),
        (linux_kernel_loc * Decimal::new(1e9), "MegaLK".to_string()),
        (linux_kernel_loc * Decimal::new(1e12), "GigaLK".to_string()),
        (linux_kernel_loc * Decimal::new(1e15), "TeraLK".to_string()),
        (linux_kernel_loc * Decimal::new(1e18), "PetaLK".to_string()),
        (linux_kernel_loc * Decimal::new(1e21), "ExaLK".to_string()),
    ];
    for (limit, unit) in stages {
        if decimal < limit {
            return format!("{} {}", (decimal / (limit / Decimal::new(1e3))).to_fixed(2), unit)
        }
    }
